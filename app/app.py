#!/usr/bin/env python3
"""Billard Stream — Lokal GUI App. Streaming til YouTube, Twitch, Facebook m.v."""
import tkinter as tk
from tkinter import ttk, messagebox
import subprocess, threading, json, os, time, platform, urllib.request, urllib.error, ssl
...
# Streaming-tjenester — URL er altid den samme, kun key ændrer sig
STREAM_SERVICES = {
    "YouTube":     "rtmp://a.rtmp.youtube.com/live2/{KEY}",
    "Twitch":      "rtmp://live.twitch.tv/app/{KEY}",
    "Facebook":    "rtmp://live-api-s.facebook.com:80/rtmp/{KEY}",
    "Custom":      "",
}

# SSL context — ignorer certifikatfejl (Cloudflare mm.)
...
class BillardApp:
    ...
    def build_ui(self):
        ...
        tk.Label(top, text="🎱 Billard Stream", font=("Arial", 16, "bold"), fg=accent, bg=bg).pack(side="left")
        tk.Label(top, text=f"v{VERSION}", font=("Arial", 9), fg="#666", bg=bg).pack(side="left", padx=10)

        # Licens
        lf = tk.LabelFrame(self.root, text="🔑 Licens", fg=accent, bg=bg, font=("Arial", 10), padx=10, pady=10)
        lf.pack(fill="x", padx=15, pady=5)
        row = tk.Frame(lf, bg=bg)
        row.pack(fill="x")
        tk.Label(row, text="Licensnøgle:", fg=fg, bg=bg, font=("Arial", 9)).pack(side="left")
        self.license_entry = tk.Entry(row, width=30, bg=entry_bg, fg=fg, insertbackground=fg, relief="flat", font=("Arial", 9))
        ...
        self.license_btn = tk.Button(row, text="✅ Valider", command=self.validate_license,
            bg=btn_bg, fg="#0a0a0a", font=("Arial", 8, "bold"), relief="flat", padx=10, cursor="hand2")
        ...
        # Stream
        sf = tk.LabelFrame(self.root, text="📡 Stream", fg=accent, bg=bg, font=("Arial", 10), padx=10, pady=5)
        sf.pack(fill="x", padx=15, pady=5)

        # Streaming-tjeneste dropdown
        r0 = tk.Frame(sf, bg=bg)
        r0.pack(fill="x", pady=2)
        tk.Label(r0, text="Tjeneste:", fg=fg, bg=bg, font=("Arial", 9), width=14, anchor="w").pack(side="left")
        self.service_combo = ttk.Combobox(r0, values=list(STREAM_SERVICES.keys()), state="readonly", width=20, font=("Arial", 9))
        self.service_combo.set("YouTube")
        ...
        self.cam_type = ttk.Combobox(r1, values=["Indbygget", "USB Kamera", "IP Kamera"], state="readonly", width=20, font=("Arial", 9))
        ...
        self.key_entry = tk.Entry(r2, width=40, bg=entry_bg, fg=fg, insertbackground=fg, relief="flat", font=("Arial", 9))
        ...
        self.ip_entry = tk.Entry(r4, width=40, bg=entry_bg, fg=fg, insertbackground=fg, relief="flat", font=("Arial", 9))

        # Knapper
        btn_frame = tk.Frame(self.root, bg=bg)
        btn_frame.pack(fill="x", padx=15, pady=10)
        self.start_btn = tk.Button(btn_frame, text="▶ START STREAM", command=self.start_stream,
            bg=btn_bg, fg="#0a0a0a", font=("Arial", 11, "bold"), relief="flat", padx=20, pady=8, cursor="hand2")
        self.start_btn.pack(side="left", padx=5)
        self.stop_btn = tk.Button(btn_frame, text="⏹ STOP", command=self.stop_stream,
            bg="#441111", fg="#ff4444", font=("Arial", 11, "bold"), relief="flat", padx=20, pady=8, cursor="hand2", state="disabled")
        self.stop_btn.pack(side="left", padx=5)
        self.status_label = tk.Label(btn_frame, text="● Slukket", fg="#888", bg=bg, font=("Arial", 10, "bold"))
        self.status_label.pack(side="right", padx=10)

        # Log
        log_frame = tk.LabelFrame(self.root, text="📋 Log", fg=accent, bg=bg, font=("Arial", 10), padx=5, pady=5)
        log_frame.pack(fill="both", expand=True, padx=15, pady=(5,15))
        self.log_text = tk.Text(log_frame, height=10, bg="#111", fg="#ccc", font=("Consolas", 9), relief="flat", wrap="word")
    ...
    def validate_license(self, silent=False):
        ...
        if not key:
            if not silent: messagebox.showerror("Fejl", "Indtast en licensnøgle")
            return
        ...
        def check():
            try:
                ...
                if data.get("valid"):
                    ...
                    self.license_valid = False
                    self.root.after(0, lambda: self.license_status.config(text=f"❌ {data.get('error', 'Ugyldig')}", fg="#ff4444"))
    ...
    def update_buttons(self):
        self.start_btn.config(state="normal" if self.license_valid else "disabled")

    def start_stream(self):
        key = self.key_entry.get().strip()
        if not key:
            messagebox.showerror("Fejl", "Indtast din stream key")
            return
        service = self.service_combo.get()
        template = STREAM_SERVICES.get(service, "")
        if not template and service == "Custom":
            messagebox.showerror("Fejl", "Vælg en tjeneste eller indtast custom RTMP URL")
            return
        ...
        if cam_type == "IP Kamera":
            ...
            cmd += ["-f", "dshow" if os_name == "Windows" else "v4l2", "-i", device]
